...
public static class TimetableAnalyzer
{
    public static AnalysisResult Analyze(RailProject project)
    {
        ...
        var intervals = new List<OccupancyInterval>();
        var stations = project.Stations.ToDictionary(s => s.Id);
        ...
        foreach (var train in project.Trains)
        {
            ...
            for (var i = 0; i < train.Timetable.Count - 1; i++)
            {
                ...
                if (tracks.Count == 0) { result.Warnings.Add($"{train.Number}: no track in required direction on {sa.Code}->{sb.Code}."); continue; }
                // One run occupies a selected directional track. In this first MVP the first matching track is used.
                intervals.Add(new OccupancyInterval(tracks[0].Id, train.Id, train.Number, start.Value, end.Value, section.Id));
            }
        }
        ...
        foreach (var group in intervals.GroupBy(i => i.TrackId))
        {
            ...
            for (var i = 0; i < ordered.Count; i++)
                for (var j = i + 1; j < ordered.Count; j++)
                {
                    ...
                    if (ordered[i].TrainId == ordered[j].TrainId) continue;
                    var start = ordered[i].Start > ordered[j].Start ? ordered[i].Start : ordered[j].Start;
                    var end = ordered[i].End < ordered[j].End ? ordered[i].End : ordered[j].End;
        ...
        foreach (var station in project.Stations)
        {
            ...
            foreach (var train in project.Trains)
                foreach (var e in train.Timetable.Where(e => e.StationId == station.Id && e.Kind == StopKind.Stop))
                {
                    var arrival = Parse(e.Arrival, e.Departure); var departure = Parse(e.Departure, e.Arrival);
                    if (arrival is not null) events.Add((arrival.Value, 1));
                    if (departure is not null) events.Add((departure.Value, -1));
                }
            var used = 0; var max = 0;
            foreach (var e in events.OrderBy(e => e.Time).ThenBy(e => e.Delta)) { used = Math.Max(0, used + e.Delta); max = Math.Max(max, used); }
            result.StationLoads.Add(new StationLoad(station.Id, max, station.Tracks));
        }
        return result;
    }

    private static TimeSpan? Parse(string primary, string fallback)
    {
        if (TimeSpan.TryParse(primary, out var value)) return value;
        if (TimeSpan.TryParse(fallback, out value)) return value;
        return null;
    }
